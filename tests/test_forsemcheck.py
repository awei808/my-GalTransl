...
class SemcheckEchoDetectTests(unittest.TestCase):
    """_is_echo_response（基类实现）：整批回显（复制输入全部标错）判定阈值。"""
    ...
    def test_small_batch_exempt(self) -> None:
        ...
        self.assertFalse(obj._is_echo_response(3, 3))
        self.assertFalse(obj._is_echo_response(4, 5))
    ...
    def test_threshold_attribute_effective(self) -> None:
        ...
        self.assertFalse(obj._is_echo_response(36, 40))  # 90% 不再触发
        obj._echo_hit_ratio = 0.5
        self.assertTrue(obj._is_echo_response(20, 40))  # 50% 触发


class SemcheckEchoBatchTests(unittest.IsolatedAsyncioTestCase):
    """模型整批回显时：丢弃本批标记、追加纠正提示重试一次；仍回显则告警。"""

    def _make_obj(self, llm_responses: list) -> ForSemCheck:
        obj = object.__new__(ForSemCheck)
        obj._log_tag = "[语义检测]"
        obj._disabled_reason = ""
        obj.pj_config = _FakeBatchConfig({})
        ...
        async def fake_llm(messages, filename, idx_tip, cb):
            ...
            resp = llm_responses[min(len(obj._llm_calls) - 1, len(llm_responses) - 1)]
            return resp, None

        obj._call_llm = fake_llm
        return obj

    async def _run(self, llm_responses: list, total: int = 40) -> ForSemCheck:
        obj = self._make_obj(llm_responses)
        targets = [_trans(i) for i in range(1, total + 1)]
        with patch.object(
            ForSemCheck, "_filter_target_translations", return_value=targets
        ):
    ...
    @staticmethod
    def _echo_resp(total: int = 40) -> str:
        return "\n".join(
            f'x{i:02d}|{{"id": {i}, "reason": "疑似错误"}}' for i in range(1, total + 1)
        )

    async def test_echo_then_success_keeps_nothing_and_no_error(self) -> None:
        # 第一次回显 → 追加纠正提示重试 → 第二次空块：标记全空、无错误记录、调用 2 次
        obj = await self._run([self._echo_resp(), "```jsonline\n\n```"])
        ...
        self.assertIn("整批回显", obj._llm_calls[1][-1]["content"])

    async def test_echo_then_echo_discards_and_warns(self) -> None:
        # 两次都回显：标记全部丢弃、记录 1 条告警、调用 2 次
        obj = await self._run([self._echo_resp(), self._echo_resp()])
        self.assertTrue(all(t.suspected_error == "" for t in obj._targets))
        self.assertEqual(len(obj._recorded_errors), 1)
        self.assertIn("重试仍疑似整批回显", obj._recorded_errors[0][2])
        self.assertEqual(len(obj._llm_calls), 2)

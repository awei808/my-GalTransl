...
验证：
  - _resolve_after_translation_mode：none / improve / brfix / improve+brfix
    以及缺省回退 enableBetterTranslation（true→improve）、非法值回退 none。
...
from GalTransl.Frontend.LLMTranslate import (
    _resolve_after_translation_mode,
    _run_after_trans_single_file,
)


def _make_projectConfig(after=None, enable_better=False):
...
class ResolveModeTests(unittest.TestCase):
    def test_none_explicit(self):
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(after="none")), "none"
        )

    def test_improve(self):
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(after="improve")), "improve"
        )

    def test_brfix(self):
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(after="brfix")), "brfix"
        )

    def test_combo(self):
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(after="improve+brfix")),
            "improve+brfix",
        )

    def test_combo_reversed_kept(self):
        # 组合顺序由配置决定，函数只做白名单过滤不重排
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(after="brfix+improve")),
            "brfix+improve",
        )

    def test_fallback_enableBetterTranslation_true(self):
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(enable_better=True)),
            "improve",
        )

    def test_default_none_when_absent(self):
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig()), "none"
        )

    def test_invalid_value_falls_back_none(self):
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(after="foobar")), "none"
        )

    def test_partial_invalid_filtered(self):
        # 仅保留白名单 token
        self.assertEqual(
            _resolve_after_translation_mode(_make_projectConfig(after="improve+bogus")),
            "improve",
        )

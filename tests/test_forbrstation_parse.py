# -*- coding: utf-8 -*-
"""ForBRStation 换行修复解析修复 A/B/C 的单元测试。

覆盖：
  - 修复 C：单行 JSON 后尾随垃圾（如 </br>、；）不丢句（raw_decode 容错）
  - 修复 B：返回 (success_count, found_count)；无 better 字段时 found_count=0
  - 修复 A：多代码块合并后仍能解析（交由 batch_translate 层面的合并逻辑，
    此处直接验证 _parse_br_jsonline_text 对多行拼接输入按 id 稀疏解析）
...
def _make_parser(target_lang: str = "Chinese"):
    """绕过重型 __init__，仅装配 _parse_br_jsonline_text 所需属性。"""
...
class DecodeJsonPartTests(unittest.TestCase):
    def test_clean_json(self):
        self.assertEqual(
            ForBRStation._decode_json_part('{"id": 1, "better": "好"}'),
            {"id": 1, "better": "好"},
        )
    ...
    def test_trailing_br_garbage(self):
        ...
        self.assertEqual(
            ForBRStation._decode_json_part(
                '{"id": 94, "better": "abc"}</br>'
            ),
            {"id": 94, "better": "abc"},
        )
    ...
    def test_trailing_punctuation(self):
        self.assertEqual(
            ForBRStation._decode_json_part('{"id": 1, "better": "x"}；'),
            {"id": 1, "better": "x"},
        )
    ...
    def test_leading_garbage(self):
        # 从首个 { 开始解析，忽略前置思考文字
        self.assertEqual(
            ForBRStation._decode_json_part('思考：{"id": 1, "better": "y"}'),
            {"id": 1, "better": "y"},
        )
    ...
    def test_brace_inside_string_kept(self):
        ...
        s = '{"id": 1, "better": "说}话"}'
        self.assertEqual(
            ForBRStation._decode_json_part(s), {"id": 1, "better": "说}话"}
        )

    def test_not_json_returns_none(self):
        self.assertIsNone(ForBRStation._decode_json_part("纯文本没有 JSON"))
        self.assertIsNone(ForBRStation._decode_json_part(""))
...
class ParseBrJsonlineTests(unittest.TestCase):
    def test_trailing_br_garbage_fix_applied(self):
        ...
        success, found = parser._parse_br_jsonline_text(line, [trans], "\r\n")
    ...
    def test_multiple_jsonlines_sparse_by_id(self):
        ...
        success, found = parser._parse_br_jsonline_text(text, trans_list, "\r\n")
    ...
    def test_missing_better_field_yields_zero_found(self):
        ...
        success, found = parser._parse_br_jsonline_text(text, [trans], "\r\n")

...
from GalTransl.Frontend.llm_standalone import (  # noqa: E402
    STANDALONE_BACKENDS,
...
async def doLLMTranslate(
    ...
    # ---- 2.7 独立引擎：后处理后端单独执行（表驱动，见 llm_standalone）----
    # 原先每类后端各写一段同构分支，ForFixRound/ForToneCheck 漏加分支后会落入
    # 主翻译流程并触发 postprocess_results 的 afterTranslation 循环，连带执行全部
    # 后处理后端。收敛为查表分发，新增后端只需在 STANDALONE_BACKENDS 加一行。
    if is_standalone_backend(eng_type):
        return await run_standalone_backend(
            projectConfig,
            eng_type,
            file_json_lists,
            ensure_model_available_if_needed,

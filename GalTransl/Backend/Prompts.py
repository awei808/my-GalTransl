...
# [br_issue_guide] 由后端运行时基于 Problem.describe_allowed_break_ends() 动态生成，
# 避免与检测侧「允许断行位置」定义不同步。
_BRSTATION_TASK = """<process_requirements>
### 任务
这是整个文件翻译完成后的【换行位置异常修复】。输入行中部分带 `problem` 字段，内容为「换行位置异常：...」，表示该句译文（dst）的换行落在了不恰当的位置——中文译文的换行只应出现在句末标点、中文逗号/顿号、空格/Tab、emoji 或颜文字之后，而当前译文把换行放在了中文词语或短语中间，导致阅读时词被错误拆断。不带 `problem` 的行仅为上下文参考。
...
2. 换行合规：修复后的译文，其 `<br>` 仅落在句末标点、中文逗号/顿号、空格/Tab、emoji 或颜文字之后，不在中文词/短语中间断行。

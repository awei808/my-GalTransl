...
######################################
# ForPlotMetaData Prompt             #
# 剧情元数据(PlotMetadata)生成专用提示词（不使用系统提示词，全部内容置于 user 消息）

FORPLOTMETA_PROMPT = """你是 Galgame 剧本分析助手。下面给出一段 Galgame 剧本文件（JSON-line 格式，每行一个 JSON 对象，含 name 与 message 字段）。请阅读全文，概括总结该文件的剧情，并将「剧情」字段压缩至 200 字以内的中文。

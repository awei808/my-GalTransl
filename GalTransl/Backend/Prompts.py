...
FORPLOTROUTE_SYSTEM_ROUTES_ONLY = "你是 Galgame 剧情结构分析专家，负责按已确定的剧情路线划分，为每条路线汇总剧情摘要。"
...
# [br_issue_guide] 与 [allowed_break_ends] 由后端运行时基于 Problem.describe_allowed_break_ends()
# 动态生成，避免与检测侧「允许断行位置」定义不同步。
_BRSTATION_TASK = """<process_requirements>
### 任务
这是整个文件翻译完成后的【换行位置异常修复】。输入行中部分带 `problem` 字段，内容为「换行位置异常：...」，表示该句译文（dst）的换行落在了不恰当的位置——中文译文的换行只应出现在[allowed_break_ends]之后，而当前译文把换行放在了中文词语或短语中间，导致阅读时词被错误拆断。不带 `problem` 的行仅为上下文参考。
...
2. 换行合规：修复后的译文，其 `<br>` 仅落在[allowed_break_ends]之后，不在中文词/短语中间断行。
3. 符号与结构：原样保留 dst 中的系统符号、控制码、句子结构和除换行外的其它符号。

### 完整示例
输入:
```
#01|{"id":1,"name":"創","dst":"%p-1;她突然站了起<br>来，跑了出去。","problem":"换行位置异常：第1行"}
...
### 输出格式
仅输出一个 ```jsonline 代码块，块外不得有任何内容。每行：复制输入行的哈希锚点（3字符+|）后接 JSON，键只能是 `id` 与 `reason`。`reason` 必填，写明具体差异类型（如"人名错译：華恋→华良"、"关键内容漏译"、"译文串行"），简短（20字内），不得只写"疑似错误"。只输出有问题的句子，无问题输出空块。

输出配方：<hash_anchor>|{"id": int, "reason": "具体原因"}
</process_requirements>
"""
...
### 任务
以下句子已被第一轮语义检测标记为「疑似错误」。请逐句复核 `src`（日文原文）与 `dst`（当前译文，语言为 [TargetLang]），对**每一句**给出明确结论：
...
# 词语色彩一致性检查轮（ForToneCheck）：对照批次级元数据的「用词色彩」标注，
# 判断译文用词是否与所属区间的色彩指导（含视角/氛围）一致。仅判定、不产译文：
# 命中句由后端置 tone_issue 标记，find_problems 认领为「词语色彩不一致」。
# 依赖 pass2_cache 批次级元数据；仅注入区间色彩标注、任务说明与 input。
FORWORDTONE_SYSTEM = "你是 Galgame 译文质检员，只负责判断译文的用词色彩是否与其所在剧情区间的色彩标注一致，不修改译文、不输出译文、不润色。"

_FORWORDTONE_TASK = """<process_requirements>
### 任务
逐句检查 `dst`（当前译文，语言为 [TargetLang]）的**用词色彩**是否与该句所属区间的色彩标注一致。色彩标注来自剧情分析，包含该区间的「用词色彩」（如 口语活泼、庄重典雅、露骨感官、克制细腻）与「视角」「氛围」。

### 判为不一致的情况（仅限明显偏离）
- 用词庄重/文雅程度与标注明显相反（如标注「露骨直白」却译得含蓄委婉，或标注「庄重典雅」却满口粗俗俚语）；
- 措辞风格与标注的语气/氛围明显不符（如标注「日常轻松」却用书面公文腔）。

### 不判为不一致的情况
语义正确但表达普通（不追求风格强化）；仅在个别语气词上的轻微差异；译文与标注折中、难以明确判定时一律不输出。

### 输出格式
仅输出一个 ```jsonline 代码块，块外不得有任何内容。每行：复制输入行的哈希锚点（3字符+|）后接 JSON，键只能是 `id` 与可选 `reason`（用 [TargetLang] 简要说明偏离方式，如"标注口语活泼，译文过于书面"）。只输出明显不一致的句子，无则输出空块。

输出配方：<hash_anchor>|{"id": int, (可选)"reason": "..."}
</process_requirements>
"""
...
# 常用片假名 H 词补充（社区 H 词表缺失，2026-08 实测发现 セックス/ペニス/マラ 等漏网）
_H_WORDS_EXTRA = (
    "セックス\nクリトリス\nオナニー\nパイズリ\nイキ\nペニス\nディック\nマラ\n"
    "フェラチオ\nチンポ\nバイブ\nイラマチオ\nクンニ\n手コキ\n足コキ\nフェラ\nオナホ\n"
    "膣内\n射精\n勃起\n挿入\n絶頂\n陰茎\n乳房\n乳首\n性感帯"
...
# 批次级元数据：按 h 强度（0-1 浮点）分档的差异化翻译指导文案
#   [0, 0.25)     标准非 h：含蓄克制
#   [0.25, 0.5)   少量 h 氛围：暧昧暗示，保留张力但无直白露骨
#   [0.5, 0.75)   h 氛围浓厚无性行为：前戏/抚摸可适度露骨，不出现性行为
#   [0.75, 1]     h 氛围浓厚有性行为：露骨直译
# 禁用词表仅对 h >= 0.5 的档位（intimate/explicit）注入，与字典分流口径一致
H_BATCH_GUIDE = (
    "本批包含 H 剧情区间（h 强度 0.75 及以上，存在插入/手交/射精等性行为描写），"
    "可放开露骨的感官与身体描写，按原文强度直译，避免过度委婉导致语义丢失；"
    "但保留原文省略号/控制符营造的节奏与留白，不堆砌辞藻、不使用生僻修辞。"
)
H_INTIMATE_GUIDE = (
    "本批 H 氛围浓厚（h 强度 0.5~0.75，处于前戏、抚摸、接吻、脱衣等身体接触阶段，"
    "尚未发生插入、手交、射精等性行为），可适度放开感官与身体描写，直译原文的触感、"
    "呼吸与亲密动作；但请勿主动添加或提前性行为细节，保持前戏阶段的氛围推进。"
)
H_TENSION_GUIDE = (
    "本批含少量 H 氛围（h 强度 0.25~0.5，如暧昧、暗示、贴身、害羞、脸颊泛红等），"
    "请保留原文的暧昧与留白，用词含蓄但不过度回避，传达出若有若无的情色张力，"
    "不出现直白的性行为或露骨描写。"
)
H_BATCH_FORBIDDEN = "同时，以下词语在本项目 H 区间中属于用词不当，禁止使用：{words}。"
NORMAL_BATCH_GUIDE = (
    "本批不含 H 剧情区间（h 强度低于 0.25），请保持含蓄克制的表达，"
    "避免露骨或暧昧的用词，与原文语境和人物关系相符。"
)
...
1. 你的输出使用PSV格式，且总是先输出以下表头：
```psv
日文原词|中文翻译|备注

2. 开始输出词表
+ 如果有专有名词，则开始输出词表，每个元素之间使用|分隔，例如：
ジークフリート|齐格飞|人名，男性
アストライア|阿斯特莱亚|人名，女性
カカオの森|可可森林|地名
霊装融合|灵装融合|招式/技能
...
# terms 模式（gendic_terms_mode_design.md）：本地提取词表后，AI 仅做逐词翻译。
# 占位符 {terms}（带序号词表）与 {context_hint}（首现句消歧）在调用端替换。
GENDIC_TERMS_PROMPT = """## 任务

翻译以下日文术语表。术语来自一部日文作品，可能是人名/地名/组织/物品/招式/特殊概念等专有名词或作品相关术语。
对每个术语输出：日文原词（必须与输入完全一致）、对应中文翻译、备注（词类型，人名需推测性别）。
...
日文原词|中文翻译|备注
```
2. 与输入术语一一对应：每个输入术语一行，日文原词必须逐字回显输入，不得改写、不得合并、不得遗漏。
3. 备注为词类型：人名（推测性别）/地名/组织/物品/招式/术语/拟声 等。
4. 若某词疑似「H 相关组合」（色情相关词汇组合）或「非术语」（普通动词短语/常用词/口语），
   在备注末尾追加「疑似H」或「疑似非术语」标记（如：术语（疑似H））。
5. 无法翻译的词输出 日文原词|（无法翻译）|原因；绝不输出词表外的词。
6. 直接停止输出，不需要任何其他解释或说明。

## 上下文提示（部分术语的示例句：每条为含该词的完整句，同一词多条以｜分隔；句尾 [a-b,c-d,…] 为词在该句中的出现位置（0 起，不含末位，至多 3 处）；仅供消歧与锚定，不属于翻译范围）
{context_hint}
...
# LLM 全权提取模式（gendic.mode=llm）：AI 直接从原文块提取术语（含翻译）。
# 占位符 {chunk} 在调用端替换为压缩文本切块。
GENDIC_LLM_EXTRACT_PROMPT = """## 任务

从以下日文游戏文本片段中提取关键术语。术语指作品相关专有名词或重要设定：人名（含外号/代号）、地名、
组织/势力、物品/道具、招式/能力、特殊概念、缩写（如字母组合）等。
忽略普通常用词（日常名词/动词/形容词/语气词/代词）与拟声词。
...
日文原词|中文翻译|备注
```
2. 每个术语一行，日文原词必须与文本中的写法完全一致（含全角/半角、假名标记）。
3. 备注为词类型：人名（推测性别）/地名/组织/物品/招式/术语/缩写 等。
4. 若某词疑似「H 相关组合」（色情相关词汇组合）或「非术语」（普通动词短语/常用词/口语），
   在备注末尾追加「疑似H」或「疑似非术语」标记（如：术语（疑似H））。
5. 无法确定翻译的词输出 日文原词|（无法翻译）|原因；只输出本片段中出现的词。
6. 宁缺毋滥：不确定是否为术语的词不要输出；每块输出 5-40 个术语。
7. 直接停止输出，不需要任何其他解释或说明。
...
# 「称呼映射」字段说明段（仅当 internals.forfilemeta.address_map 开启时注入 [address_map_spec]）
_FORFILEMETA_ADDRESS_SPEC = """   - 称呼映射：本文件中角色之间的**称谓关系**（他人如何称呼某角色），数组形式，每项为一个对象。
     每项字段：
       - 被称呼者：被称呼角色的**原文名**（如 創、凛音）。
       - 称呼者：称呼方的**原文名**（谁在叫，如 華恋、凛音）；若某角色被所有人用同一称谓、无称呼差异，可省略该字段。
       - 原文：剧本中**实际出现的原文称谓**（如 創くん、創先生）。注意原文称谓可能与「被称呼者」不同（被称呼者为「創」，他人却叫「創くん」/「創先生」）。
       - 译文：该原文称谓对应的**中文译文**（如 创君、创先生）。
     要求：同一被称呼者被不同人用不同称谓时，**每个「称呼者→称谓」组合各列一条**；无称谓差异的角色也照常列出一条（仅含 被称呼者/原文/译文）。原文称谓必须与剧本写法一致，译文为其中文翻译。"""

# 「称呼映射」参考格式示例段（仅当 address_map 开启时注入 [address_map_example]）
_FORFILEMETA_ADDRESS_EXAMPLE = """,    "称呼映射": [
...
[address_map_spec]
3. 角色名、服装名、标签、称谓译文中的专名必须与下方 <glossary> 的译名保持一致。
...
4. 每个区间输出以下字段：
   ...
   - h：0 到 1 的浮点数，代表本区间的 H 强度（越高越露骨），请依据以下 5 档阈值自行取值（左闭右开，越精确越利于区分）：
     · 0 ≤ h < 0.25：标准非 H，无任何情色氛围。
     · 0.25 ≤ h < 0.5：带少量 H 氛围的对话或描述（如 暧昧、暗示、贴身、害羞、脸颊泛红 等）。
     · 0.5 ≤ h < 0.75：H 氛围浓厚，但尚未发生插入、手交、射精等性行为（如 前戏、抚摸、接吻、脱衣 等）。
     · 0.75 ≤ h ≤ 1：H 氛围浓厚，且伴有明确的性行为（插入、手交、射精 等露骨描写）。
...
只输出一个 JSON 对象，不要输出任何解释或注释：
{
    "id": "文件名",
    "批次": [
        {"区间": [1, 40], "视角": "创", "氛围": "日常轻松", "h": 0.1, "用词色彩": "口语化、活泼自然"},
        {"区间": [41, 96], "视角": "凛音", "氛围": "情欲紧张", "h": 0.9, "用词色彩": "露骨、感官、直白"}
    ]
}
...
# 注意事项
- 角色名必须使用中文译名，且同一角色在全文各处使用同一译名（首次出现后保持一致）；「人名对照表」中已有的角色，名称必须原样采用表中译名。
...
# ForPlotRouteMap routesOnly 模式：路线划分已由用户确定，
# 仅按既有「文件归属」逐路线汇总「节点剧情」，不重新划分结构、不生成 mermaid。

FORPLOTROUTE_ROUTES_ONLY_PROMPT = """你是 Galgame 剧情结构分析专家。下面给出了**已经确定**的剧情路线划分（路线名与其包含的剧本文件），以及各文件的剧情摘要。请**不要**重新划分路线、**不要**生成 mermaid 源码，只为每条既有路线汇总「节点剧情」。
...
# 任务要求
1. **逐路线梳理**：为上面列出的**每一条**路线生成一段剧情摘要（30~150 字中文），可合并该路线内多个文件的剧情。
2. 「节点剧情」的键必须与上面的路线名**完全一致**，不得新增、删除或改名。
3. 不要输出 mermaid，也不要输出「文件归属」。
...
# 注意事项
- 键须原样照抄上面的路线名（含标点与空格），不要改写。
- 未在摘要中出现的路线也要给出剧情摘要，可结合整体信息合理归纳。

...
def _find_metadata_file(input_dir: str, name: str) -> Optional[str]:
    ...
    Args:
        ...
        name: 元数据文件名（如 "FileMetaData.json"）
...
def _find_file_metadata_file(input_dir: str) -> Optional[str]:
    """在翻译项目目录中查找 FileMetaData.json（保留原接口，委托通用查找）。"""
    return _find_metadata_file(input_dir, "FileMetaData.json")

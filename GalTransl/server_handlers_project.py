...
def route_project_api(handler: Any, registry: JobRegistry, project_id: str, sub_path: str) -> None:
    ...
    if sub_path == "/cache/search":
        ...
        try:
            ...
            # 检索主体见 server_search.search_cache_entries（与 MCP 工具层共用同一实现）
            result = search_cache_entries(
                project_dir,
                payload.get("query", ""),
                field=str(payload.get("field", "all")).strip(),
                use_regex=use_regex,
                max_results=payload.get("max_results", 500),
            )
            handler._send_json({"results": result["results"], "total": result["total"]})

...
- 0.4.3：恢复命令行版本的适配 **已完成**（CLI 恢复进度条/交互提示与配置文件自动探测，新增 -c/--config、--version 参数；补齐 CLI 工具引擎 recheck 全部重检 / check-batch-size 批次划分预检 / build-output 构建输出），补充翻译指南和项目地址的内容

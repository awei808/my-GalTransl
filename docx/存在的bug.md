...
- 首页如何开始和引导用户，
- 撤销重做机制非全局，且存在bug
- 旧缓存文件的问题检测会追加问题而不是覆盖，每次跑完流程触发全文件重检问题，就会稳定触发
- common.gpt.translation_guideline的前端默认显示为basic.md，非用户选中的翻译规范文件
- 新建项目时未标注项目路径是应用程序同目录下，且第四步常用设置中的单次翻译句数已废弃
- 程序设置界面中，日志写入文件默认开启，需改为默认关闭
- 项目配置界面中，全局分析参考翻译规范、批次划分参考翻译规范、文件元数据参考翻译规范默认是开的，需要改成默认关闭
- 批次划分的提示次中有“# 翻译规范（translation_guideline）”，本应该没有括号和括号内的文本
- 项目设置界面中有两个平均分句长度阈值（长句丢失换行）problemAnalyze.avgSentenceLengthThreshold，设置项混乱
- 翻译后端中，可能会出现字典注入时结尾处多一个|的情况，原因是没有解释内容且函数执行时未做非空校验
- 点击翻译控制台后，点击查找替换/问题检测/查看备选，无法跳转
- 深色模式下，只有译文变白色，不是所有文字
- 折叠缓存的750条阀值太小，需要新增配置项用于控制，允许用户修改
- 字典管理上，添加条目远远没有原来直观
- 在左侧按钮栏新增一个后端设置快捷按钮，并且允许在设置中关闭乎显示这个快捷按钮。
- 取消思考预算 (thinking_budget)配置项
- 待观察：后端设置界面太过密集

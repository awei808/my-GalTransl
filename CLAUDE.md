...
GalTransl 是一个 Galgame（视觉小说）自动翻译工具，利用大语言模型（GPT-4、Claude、Deepseek、Sakura 等 OpenAI 兼容后端）和提示词工程进行翻译。版本 7.3.0。通过多阶段流水线处理 name-message JSON 对，将日文原文翻译为中文（或其他目标语言）脚本，并支持字典、插件、校对审核、人名表等增强能力。
...
- **后端**：Python 3.11+，`GalTransl/` 包，支持 CLI（`python -m GalTransl`）与服务端（`run_backend.py` → `server.py`）两种启动方式。
...
| 文件 | 职责 |
|---|---|
| `__init__.py` | 包入口：`LOGGER` 日志器、版本号（`7.3.0`）、目录常量、`TRANSLATOR_SUPPORTED` 翻译器清单、`LANG_SUPPORTED` 语言表、启动更新检查线程 |
...
| `server_runtime.py` | 运行时状态追踪：`RuntimeState`/`RuntimeRegistry`（服务端模式下每项目状态）、`RuntimeProgressCache`（缓存文件进度解析，retranslKey 感知）、7 阶段流水线定义与阶段映射 |
...
## 核心流水线（ForGal-full-pipeline，7 阶段）

`translator: "ForGal-full-pipeline"` 自动串联以下阶段（与 `server_runtime.PIPELINE_STAGE_NAMES` 一致，前端"流程完成情况"按此显示）：
...
各阶段均可按缓存产物是否已存在而跳过（`internals.pipeline.forceRegen*` 可强制重生成）。独立引擎（`ForGlobalPrompt`/`GenDic`/`ForFileMetaData`/`ForBatchMetaData`/`dump-name`/`show-plugs`）也可单独作为 `translator` 运行。
...
服务端模式下每项目一个，供前端 `/runtime` 轮询：`stage`/`stage_index`/`stage_total`（7 阶段）、`current_file`/`current_batch`/`batch_total`、`workers_active`/`workers_configured`、`translation_speed_lpm`（1 分钟窗口速度）、`latest_prompt_preview`/`latest_assembled_preview`（实时提示词/译文预览）、`recent_errors`/`recent_successes`（环形队列）、`notices`（一次性用户提示，前端 toast 后清除）。

### 缓存条目（CacheEntry）

缓存文件（`transl_cache/pass3_cache/*.json`）中每条 JSON 的核心字段：`index`、`name`、`pre_src`、`post_src`、`pre_dst`、`proofread_dst`、`trans_by`、`proofread_by`、`problem`、`post_dst_preview`，以及内部键 `__cache_key`（前句+当前句+后句上下文三元组，带 index 前缀区分同文不同位置）。
...
| GET | `/api/backend-profiles` / `/api/backend-profiles/:name` | 全局后端配置读取 |
...
| `write_text_file` | `{path, content}` | 写文本文件 |
| `copy_files` | `{sources[], dest}` | 批量复制文件 |

## 开发环境配置和运行指南

### 环境要求

- **Python 3.11+**（依赖版本面向 3.11）
...
- 测试文件：后端在 `tests/test_*.py`（30 个测试文件，覆盖缓存/字典/元数据/API 端点/安全/运行时进度等），前端在 `desktop/src/__tests__/`

### 调试辅助（结合测试项目 test-dev）

- **测试翻译项目**：`D:\解包或汉化用\xp3专用汉化文件夹\gal翻译\test-dev` 是常用的实际翻译样例项目，包含完整的 `gt_input`/`gt_output`/`transl_cache`、`config.yaml`（当前配置为 Deepseek/OpenRouter 双 token、random 策略）、译前/译后/项目 GPT 字典与 `name替换表.csv`。适合用它验证流水线改动后的实际翻译效果。

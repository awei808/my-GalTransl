...
8. **新建项目向导添加更多设置**：可在新建项目向导中，设置游戏外部信息、剧情大纲与剧情结构类型，设置翻译流水线各阶段是否执行等。
9. **自动保存功能**：字典、缓存元数据文件、配置文件支持自动失焦保存；译文修改界面由于功能复杂和撤销功能需要，不支持自动保存。
10. **缓存结构变化**：为适配多流程翻译（全局分析 → 文件级元数据 → 翻译执行等多阶段），翻译缓存结构做了必要调整，与旧翻译项目不兼容。
11. **翻译项目路径**：新建的翻译项目只能位于应用程序同目录下，旧项目不受此限制。
12. **仅适配 Galgame 汉化**：重点面向 galgame 文本翻译场景；epub、字幕等原项目的通用文件功能**无法正常使用**。
13. **统一模型api入口**：仅支持OpenAI格式的api调用方式，大模型与本地模型都走此借口，不再为sakura保留独有接口与翻译流程
14. **命令行已恢复适配**：命令行版本（CLI）已适配核心翻译流程，支持自动识别 `config.inc.yaml` / `config.yaml` 配置；桌面端仍是主要使用方式，CLI 属高级用户/开发者向，界面类功能（校对、字典编辑等）仅在桌面端提供。
15. **其他细小变化**：JSON 内统一使用 `\n` 换行符（替代 `\r\n`），字典统一使用 PSV 格式等。

16. **已修复的问题**：翻译控制台可能出现重复弹窗、个别文件进度显示 bug 等界面问题、撤销/重做（含跨文件）、项目切换状态残留等问题已基本完善。
  * 特性：   
  ...
  4. 通过译前（预处理）、译后（后处理）字典与条件字典实现灵活的自动化字典系统，支持**H/非H 双库 GPT 字典**、禁用词字典与**人名替换表**（CSV/XLSX）
  5. 实时保存缓存、自动断点续翻，翻译过程中可视化展示提示词、译文预览、并发进度与速度
  6. 完整流水线：输入校验 → 文本压缩 → 全局游戏分析 → 自动构建术语表 → 文件级元数据 → 剧情路线图 → 批次划分 → 翻译执行 →（可选）译后改进/修复/语义检测，多阶段分批提示词注入提升翻译质量
...
## 近期更新
* 2026.8：更新 v0.3.0，新增**语义检测**（AI 判定疑似错译/漏译/串行，命中句二次复核）、**H/非H 双库 GPT 字典**与禁用词字典、残留日文/禁用词修复引擎、定语/状语过长与 H 场景用词不当检测、跨文件撤销/重做；后端重构拆分 BaseEngine（API 客户端与翻译轮解耦）
* 2026.8：更新 v0.2.0，补齐**完整流水线**（输入校验 → 文本压缩 → 全局分析 → 术语表 → 文件级元数据 → 剧情路线图 → 批次划分 → 翻译执行）、流水线各阶段独立开关、译文改进（ForImproveTranslation）与换行修复（ForBRStation）、备选译文、长句丢失换行/换行位置异常检测
* 2026.8：个人重构 v0.1.0，使用**桌面端图形界面**（Tauri 2 + SolidJS + Rust），支持可视化翻译工作台、校对审核、后端配置统一管理、人名替换表等
...
&ensp;&ensp;&ensp;&ensp;**④ 开始翻译**：在"翻译控制台"选择后端并点击"启动流程"，实时查看翻译进度、速度、当前提示词与译文预览结果，可随时停止。
...
&nbsp;&nbsp;&nbsp;&nbsp;**阶段零 · 输入数据校验**
&nbsp;&nbsp;&nbsp;&nbsp;逐文件校验输入 JSON 的 `message`/`name` 完整性，校验失败则中止流水线（`enableValidate=false` 可关闭，不建议）。
...
&nbsp;&nbsp;&nbsp;&nbsp;**阶段三 · 术语表构建（GenDic）**
&nbsp;&nbsp;&nbsp;&nbsp;从全文提取专名/术语，自动生成项目 GPT 字典（`项目GPT字典-生成.txt`）。项目已有非空 GPT 字典时跳过（`forceRegenDic=true` 可强制重新生成）；该阶段未充分优化，可在向导中关闭。

&nbsp;&nbsp;&nbsp;&nbsp;**阶段四 · 文件级剧情元数据（ForFileMetaData）**
&nbsp;&nbsp;&nbsp;&nbsp;逐文件分析剧本，生成该文件剧情梗概与出场角色等元数据（不翻译、无多轮），写入缓存 `transl_cache/pass1_cache/{文件名}.meta.json`（存在即跳过该文件）。

&nbsp;&nbsp;&nbsp;&nbsp;**阶段 4.5 · 剧情路线图生成（ForPlotRouteMap）**
...
&ensp;&ensp;&ensp;&ensp;修改好项目设置后，确保你已经安装了需要的依赖（见环境准备），然后双击`run_GalTransl_terminal.bat`，输入项目路径即可开始翻译（会弹出引擎选择菜单）。也可以直接用命令行调用：

```bash
python -m GalTransl -p <项目目录> -t <翻译引擎> [-l info] [-c 配置文件名]
```
...
problemAnalyze:
  problemList: # 要发现的问题清单
    ...
    - 标点错漏 # 标点符号多加或漏加
    - 残留日文 # 日文平假名片假名残留
    - 丢失换行 # 缺少换行符，一般没事，配合长句丢失换行保证译文实际观感
    ...
    #- 引入英文 # 本来没有英文，译文引入了英文
    #- 比日文长严格 # 严格查找，不能比日文长
    #- 长句丢失换行 # 译文平均分句长度超过设置的阀值，疑似丢失应有换行
...
目前支持找以上问题，有的项目被#号注释，可以取消来开启，或手动加上#号关闭对应问题的查找。另外还有 **定语过长 / 状语过长**（检测"XX的"与"在XX"等超长修饰成分）与 **用词不当**（H 场景与非 H 场景按禁用词/词库命中标记）等检测项，可在配置中按需启用。
